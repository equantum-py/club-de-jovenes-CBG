...
const pillars = [
  { number: "01", title: "Palabra", text: "Crecer en el conocimiento de Dios y aprender a vivir una fe firme y práctica." },
  { number: "02", title: "Comunidad", text: "Compartir, acompañarnos y construir amistades con propósito dentro y fuera de la iglesia." },
  { number: "03", title: "Servicio", text: "Poner nuestros dones al servicio de Cristo, de la iglesia y de quienes nos rodean." },
];
      ...
      <main>
          ...
          <Container className="relative grid min-h-[680px] items-end gap-10 py-14 sm:min-h-[720px] sm:py-20 lg:grid-cols-[1.1fr_0.9fr] lg:items-center lg:py-28">
            ...
            <div className="hidden lg:flex lg:justify-end">
              <div className="max-w-sm border-l border-white/20 pl-8">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-gold">Próximo gran encuentro</p>
                <p className="mt-4 text-3xl font-semibold">Gracia Camp 2026</p>
                <p className="mt-3 leading-7 text-white/65">11 de diciembre · Atyrá, Paraguay. Una experiencia de Palabra, comunión y crecimiento.</p>
              </div>
            </div>
          </Container>
        </section>
        ...
        <section className="border-b border-brand-border bg-brand-cream">
          <Container className="grid gap-0 py-0 md:grid-cols-3">
            {pillars.map((pillar) => (
              <article key={pillar.number} className="border-b border-brand-border py-10 md:border-b-0 md:border-r md:px-8 md:first:pl-0 md:last:border-r-0 md:last:pr-0">
                <p className="text-xs font-semibold tracking-[0.2em] text-brand-gold">{pillar.number}</p>
                <h2 className="mt-3 text-2xl font-semibold text-brand-forest">{pillar.title}</h2>
        ...
        <section className="bg-brand-forest text-white">
          <Container className="grid gap-10 py-16 sm:py-20 lg:grid-cols-[1fr_auto] lg:items-end lg:py-24">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-gold">Campamento 2026</p>
              <h2 className="mt-4 max-w-4xl text-4xl font-semibold leading-tight sm:text-6xl">Gracia Camp: una fe examinada, una vida rendida a Cristo.</h2>
              <p className="mt-5 max-w-2xl text-lg leading-8 text-white/70">Toda la información, el programa, la remera oficial y el registro están disponibles en la página del campamento.</p>
            </div>
            <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
              <ButtonLink href="/campamento" variant="light">Ver campamento</ButtonLink>
              <Link href="/registro" className="inline-flex min-h-11 items-center justify-center rounded-full border border-white/25 px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/10">Inscribirme</Link>
            </div>
          </Container>
        </section>
      </main>
