...
export default async function CabanasPage(){const[cabins,registrations]=await Promise.all([listCabins().catch(()=>[]),listRegistrations().catch(()=>[])]);return <main className="p-5 sm:p-8 lg:p-10">
  ...
  <section className="mt-8 rounded-2xl border border-brand-border bg-white p-5 sm:p-7"><h2 className="text-xl font-semibold text-brand-forest">Crear cabaña</h2><form action={createAction} className="mt-5 grid gap-4 lg:grid-cols-2">
    <label className="grid gap-2 text-sm font-semibold text-brand-forest">Nombre de la cabaña<input name="name" required placeholder="Ej. Cabaña 1" className="min-h-12 rounded-xl border border-brand-border px-4 font-normal"/></label>
    <label className="grid gap-2 text-sm font-semibold text-brand-forest">Líder de cabaña<select name="leaderId" className="min-h-12 rounded-xl border border-brand-border px-4 font-normal"><option value="">Sin asignar</option>{registrations.map(r=><option key={r.id} value={r.id}>{r.nombre} {r.apellido}</option>)}</select></label>
    <label className="grid gap-2 text-sm font-semibold text-brand-forest">Capacidad<input name="capacity" type="number" min="1" placeholder="Ej. 8" className="min-h-12 rounded-xl border border-brand-border px-4 font-normal"/></label>
    <label className="grid gap-2 text-sm font-semibold text-brand-forest">Observaciones<input name="notes" placeholder="Opcional" className="min-h-12 rounded-xl border border-brand-border px-4 font-normal"/></label>
