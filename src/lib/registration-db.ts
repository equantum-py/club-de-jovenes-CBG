...
export async function saveRegistration(payload: RegistrationPayload, selfiePath = "") {
  ...
  const response = await fetch(`${url}/rest/v1/registrations`, {
    ...
    body: JSON.stringify({
      ...
      telefono: payload.telefono, cedula: payload.cedula, sexo: payload.sexo,
      ...
      alergias: payload.alergias || null, medicamentos: payload.medicamentos || null,
      ...
      observaciones: payload.observaciones || null, forma_pago: payload.formaPago,
...
export async function listRegistrations(): Promise<StoredRegistration[]> {
  ...
  return rows.map((row) => ({
    id: String(row.id ?? ""), fecha: String(row.created_at ?? ""), nombre: String(row.nombre ?? ""), apellido: String(row.apellido ?? ""),
    edad: String(row.edad ?? ""), telefono: String(row.telefono ?? ""), cedula: String(row.cedula ?? ""), sexo: String(row.sexo ?? ""),
    iglesia: String(row.iglesia ?? ""), esInvitado: row.es_invitado ? "Sí" : "No", invitadoPor: String(row.invitado_por ?? ""),
    alergias: String(row.alergias ?? ""), medicamentos: String(row.medicamentos ?? ""), enfermedadBase: String(row.enfermedad_base ?? ""),
    contactoEmergenciaNombre: String(row.contacto_emergencia_nombre ?? ""), contactoEmergenciaTelefono: String(row.contacto_emergencia_telefono ?? ""),
    observaciones: String(row.observaciones ?? ""), formaPago: String(row.forma_pago ?? ""), nombrePadreMadre: String(row.nombre_padre_madre ?? ""),
    telefonoPadreMadre: String(row.telefono_padre_madre ?? ""), estado: String(row.estado ?? "registrado"), selfiePath: String(row.selfie_path ?? ""),
  }));
}
